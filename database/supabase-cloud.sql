...
alter table if exists public.qc_record
  add column if not exists disposal_status varchar(32) default 'OPEN',
  add column if not exists disposal_assignee varchar(100),
  add column if not exists disposal_close_reason text,
  add column if not exists disposal_updated_at timestamp;
...
grant select, insert, update, delete on table public.workflow_definition, public.workflow_instance, public.workflow_task, public.workflow_log to anon, authenticated;
grant usage, select on all sequences in schema public to anon, authenticated;
