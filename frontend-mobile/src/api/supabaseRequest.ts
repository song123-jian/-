...
async function buildInjectionMobileTasks(params?: Record<string, any>) {
  const moduleKey = String(params?.moduleKey || '').trim()
  const modules = moduleKey ? INJECTION_MODULES.filter((item) => item.key === moduleKey) : INJECTION_MODULES
  const results = await Promise.allSettled(
    modules.map(async (module) => {
      const route = resolveResourceRoute(module.resource)
      if (!route) return []
      const res = await listTable(route, { page: 1, pageSize: 20 })
      const records = (res.data.records || []).filter((row: any) => {
        const status = String(row.status || '').toUpperCase()
        return !['CLOSED', 'FINISHED', 'APPROVED_PRODUCTION', 'PASSED'].includes(status)
      })
      return records.map((row: any) => ({
        id: `${module.key}-${row.id}`,
        moduleKey: module.key,
        moduleTitle: module.shortTitle,
        title: row[module.codeField] || row.title || row.name || `${module.shortTitle}${row.id}`,
        description: row.remark || row.description || module.description,
        route: `/m/injection/${module.key}`,
        status: row.status || '',
      }))
    }),
  )
  return ok(results.flatMap((item) => item.status === 'fulfilled' ? item.value : []))
}
...
async function get(url: string, config?: RequestConfig) {
  ...
  if (path === 'injection/mobile-tasks') return buildInjectionMobileTasks(config?.params)
