    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="80px">
        <el-form-item label="供应商" prop="supplierId">
          <el-input v-model="form.supplierId" placeholder="请选择供应商" />
        </el-form-item>
        ...
        <el-form-item label="入库日期" prop="inDate">
          <el-date-picker v-model="form.inDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
        </el-form-item>
...
import { stockInPurchase } from '@/api/stock'
...
const form = reactive({ id: 0, supplierId: '', materialId: '', quantity: 1, warehouseId: '', inDate: '', remark: '' })
...
function handleAdd() {
  ...
  Object.assign(form, { id: 0, supplierId: '', materialId: '', quantity: 1, warehouseId: '', inDate: '', remark: '' })
  dialogVisible.value = true
}
function handleEdit(row: any) { dialogTitle.value = '编辑采购入库'; Object.assign(form, row); dialogVisible.value = true }

async function handleSubmit() {
  const valid = await formRef.value?.validate().catch(() => false)
  if (!valid) return
  try {
    await stockInPurchase(form)
