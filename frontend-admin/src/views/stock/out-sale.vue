    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="80px">
        <el-form-item label="销售订单" prop="saleOrderId">
          <el-input v-model="form.saleOrderId" placeholder="请输入销售订单" />
        </el-form-item>
...
import { stockOutSale } from '@/api/stock'
...
const form = reactive({ id: 0, saleOrderId: '', productId: '', quantity: 1, warehouseId: '', remark: '' })
const formRules: FormRules = {
  saleOrderId: [{ required: true, message: '请输入销售订单', trigger: 'blur' }],
...
function handleAdd() {
  ...
  Object.assign(form, { id: 0, saleOrderId: '', productId: '', quantity: 1, warehouseId: '', remark: '' })
  dialogVisible.value = true
}
function handleEdit(row: any) { dialogTitle.value = '编辑销售出库'; Object.assign(form, row); dialogVisible.value = true }

async function handleSubmit() {
  const valid = await formRef.value?.validate().catch(() => false)
  if (!valid) return
  try { await stockOutSale(form); ElMessage.success('操作成功'); dialogVisible.value = false; fetchData() } catch { /* */ }
}

async function handleDelete(row: any) {
  await ElMessageBox.confirm('确定删除该出库记录？', '提示', { type: 'warning' })
