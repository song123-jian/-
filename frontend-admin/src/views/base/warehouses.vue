<template>
  <MasterCrudPage :title="config.title" :metrics="metrics">
    ...
    <template #detail>
      <BaseCrudDetail :title="config.detailTitle" :empty-text="config.emptyDetailText" :row="selectedWarehouse" :items="config.detailItems" />
    </template>
...
import { createWarehouse, deleteWarehouse, getWarehouseList, updateWarehouse } from '@/api/warehouse'
import { warehousePageConfig as config } from '@/views/base/base-data-schema'
...
const {
  ...
  deleteConfirmText: (row) => `确认删除仓库 ${row.name}？`,
})

function handleRowAction(key: string, row: any) {
  if (key === 'inspect') return (selectedWarehouse.value = row)
