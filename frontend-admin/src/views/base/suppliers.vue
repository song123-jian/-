  ...
  <MasterCrudPage :title="config.title" :subtitle="config.subtitle" :metrics="metrics">
    ...
    <template #search>
      <SearchBar
        ...
        title="筛选供应商"
        description="按供应商关键字、状态和风险等级快速定位采购伙伴。"
        @search="handleSearch"
        @reset="handleReset"
      >
        <template #default>
    ...
    <template #detail>
      <BaseCrudDetail :title="config.detailTitle" :empty-text="config.emptyDetailText" :row="selectedSupplier" :items="config.detailItems" />
    </template>
...
import { createSupplier, deleteSupplier, getSupplierList, updateSupplier } from '@/api/supplier'
import { supplierPageConfig as config } from '@/views/base/base-data-schema'
...
const {
  ...
  deleteConfirmText: (row) => `确认删除供应商 ${row.name}？`,
})

function handleRowAction(key: string, row: any) {
  if (key === 'inspect') return (selectedSupplier.value = row)
