...
const form = reactive({ id: 0, code: '', name: '', contact: '', phone: '', address: '' })
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getCustomerList({ page: pagination.page, pageSize: pagination.pageSize })
...
function handleAdd() {
  ...
  Object.assign(form, { id: 0, code: '', name: '', contact: '', phone: '', address: '' })
  dialogVisible.value = true
}
function handleEdit(row: any) { dialogTitle.value = '编辑客户'; Object.assign(form, row); dialogVisible.value = true }

async function handleSubmit() {
  const valid = await formRef.value?.validate().catch(() => false)
  if (!valid) return
  try {
    if (form.id) { await updateCustomer(form.id, form); ElMessage.success('更新成功') }
    else { await createCustomer(form); ElMessage.success('创建成功') }
...
async function handleDelete(row: any) {
  ...
  try { await deleteCustomer(row.id); ElMessage.success('删除成功'); fetchData() } catch { /* */ }
}
